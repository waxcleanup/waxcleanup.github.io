...
// -------------------- memos --------------------
//
// IMPORTANT: router.hpp infers proposer from transfer.from.
// Do NOT embed proposer in the memo.
//
// ✅ Template burn proposal:
// proposeburn:<collection>:<schema>:<template_id>:<trash_raw>:<cinder_raw>:<cap>
//
// ✅ Schema burn proposal:
// proposeschemaburn:<collection>:<schema>:0:<trash_raw>:<cinder_raw>:<cap>

// proposeburn:<collection>:<schema>:<template_id>:<trash_raw>:<cinder_raw>:<cap>
function buildProposeBurnMemo({ collection, schema, template_id, trash_fee, cinder_reward, cap }) {
  ...
  return `proposeburn:${collection}:${schema}:${tpl}:${trash_raw}:${cinder_raw}:${capInt}`;
}

// proposeschemaburn:<collection>:<schema>:0:<trash_raw>:<cinder_raw>:<cap>
function buildProposeSchemaBurnMemo({ collection, schema, trash_fee, cinder_reward, cap }) {
  mustNonEmpty(collection, 'collection');
  mustNonEmpty(schema, 'schema');
  ...
  // template_id MUST be 0 for schema burn proposals
  return `proposeschemaburn:${collection}:${schema}:0:${trash_raw}:${cinder_raw}:${capInt}`;
}
...
/**
 * Submit a proposal
 * Token transfer -> cleanupcentr with correct proposal memo
 *
...
export const submitProposal = async ({
  ...
  // schema is required for BOTH types now (router expects it)
  mustNonEmpty(schema, 'schema');
