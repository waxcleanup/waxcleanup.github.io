...
export const burnNFT = async (accountName, nft, incinerator) => {
  ...
  // 🔐 Build encrypted payload for the backend
  const burnPayload = {
    type: 'burn',
    user: accountName,
    assetId: String(nft.asset_id),
    incineratorId: String(incinerator.asset_id || incinerator.id),
    ts: Math.floor(Date.now() / 1000), // unix timestamp (seconds)
  };

  const encryptedMemo = encryptBurnPayload(burnPayload);
    ...
    actions: [
      {
        ...
        data: {
          ...
          memo: encryptedMemo, // 🔒 now encrypted: ENC:<iv>:<cipher>
        },
      },
    ],
  };
