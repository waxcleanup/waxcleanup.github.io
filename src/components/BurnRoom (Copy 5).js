...
const BurnRoom = ({ accountName, onClose }) => {
  ...
  useEffect(() => {
    if (accountName) {
      setLoading(true);
      Promise.all([fetchBurnableNFTs(accountName), fetchProposals(), fetchStakedAndUnstakedIncinerators(accountName)])
  ...
  const handleRemoveIncinerator = (slotIndex) => {
    ...
    if (removedIncinerator) {
      setUnstakedIncinerators((prev) => [...prev, removedIncinerator]);
    }
  };
  ...
  const handleIncineratorSelect = (incinerator) => {
    ...
    setUnstakedIncinerators((prev) => [...prev.filter((i) => i.asset_id !== incinerator.asset_id), incinerator]);
  };
  ...
  const refreshIncinerators = async () => {
    try {
      setLoading(true);
