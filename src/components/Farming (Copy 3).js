...
import { equipTool, unequipTool } from '../services/toolEquipActions'; // NEW
...
export default function Farming() {
  ...
  // Track pending equip/unequip actions
  const [toolPending, setToolPending] = useState(null); // NEW
  ...
  const handlePlantSlot = async ({
    ...
    try {
      ...
      // Refresh farms (slots) and player status (seeds/compost)
      ...
      // optional: set a UI error state or toast
    }
  };
  ...
  const handleUnequipTool = async (slot) => {
    ...
    try {
      ...
      await unequipTool({
        actor: wallet,
        slot,          // "watering" or "harvesting"
      });


      // Reload inventory to reflect unequipped tool
  ...
  const handleEquipTool = async (slotType, toolAssetId) => {
    ...
    try {
      setToolPending(`equip-${slotType}`); // still useful for UI state
      await equipTool({
        actor: wallet,
        toolAssetId, // service sends { owner, asset_id }
      });
    ...
    <div className="farming-container">
      ...
      {inventory && (
        <CharacterLoadout
          ...
          onUnequipTool={handleUnequipTool}  // NEW: for unequip buttons
          toolPending={toolPending}         // NEW: show "Unequipping..." state
        />
      )}

      {/* Bag (wallet NFTs: seeds, compost, tools, cores, etc.) */}
      <BagPanel wallet={wallet} />
      {/* ^ back to original so your bag rendering logic works like before */}
